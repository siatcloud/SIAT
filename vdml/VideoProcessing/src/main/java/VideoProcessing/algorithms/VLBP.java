	...
	public long[] motion_binary_pattern(int[][] preFrame, int[][] currentFrame, int[][] nextFrame) throws IOException {
		...
		for (int i = 2; i <= w - 2; i++) {
			for (int j = 2; j <= h - 2; j++) {
				...
				/*
				 * if(array[i-1][j-1]>=j0){ sec_lay1[i-1][j-1]=1; //out.write(array[i-1][j-1]+
				 * " "+j0 +" "+sec_lay1[i-1][j-1]+" "); }
				 */
				...
				/*
				 * if(array[i+1][j+1]>=j0){ sec_lay1[i+1][j+1]=1; //out.write(I3[i+1][j+1]+
				 * " ");
				 * 
				 * }
				 */
				...
				/*
				 * if(array2[i-1][j-1]>=j0){ sec_lay2[i-1][j-1]=1; //out.write(array3[i-1][j-1]+
				 * " "+j0 +" "+sec_lay2[i-1][j-1]+" "); //out.write(I3[i-1][j-1]+ " "); }
				 */
				...
				/*
				 * if(array2[i-1][j+1]>=j0){ sec_lay2[i-1][j+1]=1; //out.write(I3[i-1][j+1]+
				 * " "); }
				 */
				...
				/*
				 * if(array3[i-1][j-1]>=j0){ sec_lay3[i-1][j-1]=1; //out.write(array3[i-1][j-1]+
				 * " "+j0 +" "+sec_lay2[i-1][j-1]+" "); //out.write(I3[i-1][j-1]+ " "); }
				 */
				if (nextFrame[i][j] >= j0) {
					sec_lay3[i][j] = 8192;
				...
				/*
				 * if(array3[i-1][j+1]>=j0){ sec_lay3[i-1][j+1]=1; //out.write(I3[i-1][j+1]+
				 * " "); }
				 */
		...
		int pixel;
		/*
		 * for (int j = 0; j < 781; j++) { for (int col = 0; col < 144; col++) { for
		 * (int row = 0; row < 180; row++) { pixel = LBP[col][row];
		 * out.write(LBP[col][row]+" "); if (pixel == j) { f[j]++; } } out.newLine(); }
		 * }
		 */
		/*
